...
const readDriverSession = (): StoredDriverSession | null => {
  ...
  for (const key of possibleSessionKeys) {
    ...
    try {
      ...
      const driverId = Number(
        parsed?.driverId ?? parsed?.DriverId ?? parsed?.id ?? parsed?.Id,
      );
      ...
      if (Number.isFinite(driverId) && driverId > 0) {
        return {
          ...
          phone: parsed?.phone ?? parsed?.Phone ?? parsed?.phoneNumber ?? '',
        };
      }
    } catch {
   ...
   */
  const rawDriverId =
    localStorage.getItem('driverId') ?? sessionStorage.getItem('driverId');
  ...
  if (Number.isFinite(driverId) && driverId > 0) {
    return {
      driverId,
      fullName:
        localStorage.getItem('driverFullName') ??
        sessionStorage.getItem('driverFullName') ??
        '',

      phone:
        localStorage.getItem('driverPhone') ??
        sessionStorage.getItem('driverPhone') ??
        '',
    };
  }
...
const normalizeDispatchStatus = (status: unknown): DriverDispatchStatus => {
  if (status === 1 || status === '1' || status === 'Assigned') {
    return 'Assigned';
  }
...
const getVehiclePlate = (vehicle: Vehicle | null): string => {
  if (!vehicle) {
    return '';
  }

  const source = vehicle as Vehicle & {
...
const getVehicleName = (vehicle: Vehicle | null): string => {
  if (!vehicle) {
    return 'خودرو';
  }
  ...
  return `خودرو ${vehicle.id.toLocaleString('fa-IR')}`;
};

const getVehicleType = (vehicle: Vehicle | null): number | null => {
  if (!vehicle) {
    return null;
  }
...
const getDispatchNumber = (dispatch: Dispatch, key: string): number | null => {
  const source = dispatch as unknown as Record<string, unknown>;
...
const getDispatchString = (dispatch: Dispatch, key: string): string => {
  const source = dispatch as unknown as Record<string, unknown>;
...
const mapActiveDispatch = (
  ...
  const vehicleId = getDispatchNumber(source, 'vehicleId') ?? 0;

  const createdAt = getDispatchString(source, 'createdAtUtc');
  ...
  return {
    ...
    title:
      ...
      `مأموریت ${source.id.toLocaleString('fa-IR')}`,

    description: getDispatchString(source, 'description') || null,
    ...
    originTitle: getDispatchString(source, 'originTitle') || 'مبدأ',

    originLatitude: getDispatchNumber(source, 'originLatitude'),

    originLongitude: getDispatchNumber(source, 'originLongitude'),

    destinationTitle: getDispatchString(source, 'destinationTitle') || 'مقصد',

    destinationLatitude: getDispatchNumber(source, 'destinationLatitude'),

    destinationLongitude: getDispatchNumber(source, 'destinationLongitude'),
...
export function DriverPortalClient() {
  ...
  const [session, setSession] = useState<StoredDriverSession | null>(null);

  const [dispatch, setDispatch] = useState<DriverActiveDispatch | null>(null);
  ...
  useEffect(() => {
    const driverSession = readDriverSession();
    ...
    const loadPortal = async () => {
      try {
        ...
        const activeDispatch = await dispatchService.getActiveForDriver(
        ...
        const vehicleId = getDispatchNumber(activeDispatch, 'vehicleId');

        let currentVehicle: Vehicle | null = null;

        if (vehicleId && vehicleId > 0) {
          currentVehicle = await vehicleService.getById(
            vehicleId,
            controller.signal,
          );
        }
        ...
        setDispatch(mapActiveDispatch(activeDispatch, currentVehicle));
      } catch (error: unknown) {
        if (controller.signal.aborted) {
          return;
        }
    ...
    const loadHistory = async () => {
      try {
        ...
        const items = await dispatchService.getDriverHistory(
          driverSession.driverId,
          controller.signal,
        );
  ...
  const profile = useMemo<DriverProfile>(() => {
    const fullName = session?.fullName || 'راننده SunPath';
    ...
    return {
      ...
      driverCode: session?.driverId
        ? `DRV-${String(session.driverId).padStart(4, '0')}`
        : '—',
      ...
      vehicleCount: new Set(
        [
          ...history.map((item) => item.vehicleId),

          ...(vehicle ? [vehicle.id] : []),
        ].filter((id) => id > 0),
      ).size,

      completedDispatches: history.length,
  ...
  const reloadHistory = async (): Promise<void> => {
    ...
    try {
      ...
      const items = await dispatchService.getDriverHistory(session.driverId);
    ...
    return (
      <main className="flex min-h-dvh items-center justify-center bg-neutral-50 px-4 text-neutral-900 dark:bg-neutral-950 dark:text-neutral-50">
        <section className="w-full max-w-md rounded-3xl border border-neutral-200 bg-white p-6 text-center shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
          <h1 className="text-lg font-black">اطلاعات ورود پیدا نشد</h1>

          <p className="mt-3 text-sm leading-7 text-neutral-500 dark:text-neutral-400">
            شناسه راننده در مرورگر ذخیره نشده است. لطفاً از صفحه ورود راننده
      ...
      <div className="mx-auto flex min-h-dvh w-full max-w-[1600px]">
        ...
        <div className="min-w-0 flex-1 pb-24 lg:pb-0">
          ...
          <div className="mx-auto w-full max-w-6xl px-4 py-5 sm:px-6 lg:px-8 lg:py-8">
            {activeTab === 'dispatch' &&
              (dispatch ? (
                ...
                <section className="rounded-3xl border border-neutral-200 bg-white p-8 text-center dark:border-neutral-800 dark:bg-neutral-900">
                  <p className="font-black">مأموریت فعالی ندارید</p>

                  <p className="mt-2 text-sm text-neutral-500 dark:text-neutral-400">
                    پس از تخصیص مأموریت توسط مدیر ناوگان، اطلاعات آن در این بخش
                    نمایش داده می‌شود.
                  </p>
                </section>
              ))}
            ...
            {activeTab === 'route' &&
              (dispatch ? (
                <DriverRouteView dispatch={dispatch} />
              ) : (
                <section className="rounded-3xl border border-neutral-200 bg-white p-8 text-center dark:border-neutral-800 dark:bg-neutral-900">
                  <p className="font-black">مسیر فعالی وجود ندارد</p>

                  <p className="mt-2 text-sm text-neutral-500 dark:text-neutral-400">
                    برای نمایش مسیر، ابتدا باید مأموریتی به شما تخصیص داده شود.
                  </p>
                </section>
              ))}
