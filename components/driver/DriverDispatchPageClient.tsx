...
const DriverNavigationMap = dynamic(
  () => import('@/components/driver/DriverNavigationMap'),
  {
    ssr: false,
    loading: () => (
...
const toMission = (d: Dispatch | null): ActiveMission | null => {
  ...
  return {
    ...
    status:
      status === '2' || status === 'started' || status === 'inprogress'
        ? 'in_progress'
...
export function DriverDispatchPageClient() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setIsLoading(true);
      try {
        const active = await dispatchService.getActiveForDriver(
          driver.driverId,
        );
        if (cancelled) return;
        setDispatch(active);
          ...
          {isLoading ? (
            ...
            <section className="rounded-3xl border border-neutral-200 bg-white p-8 text-center dark:border-neutral-800 dark:bg-neutral-900">
              ...
              <p className="mt-2 text-xs leading-6 text-neutral-500 dark:text-neutral-400">
                پس از تخصیص مأموریت توسط مدیر، مسیر و مقصد در همین پنل نمایش
                داده می‌شود.
              </p>
            </section>
          ) : activeTab === 'dispatch' ? (
            <div className="flex flex-col gap-5">
                ...
                <div className="space-y-4 p-5">
                  ...
                  <div className="grid grid-cols-2 gap-3 border-t border-neutral-100 pt-4 dark:border-neutral-800">
                    <InfoRow
                      ...
                      value={`${navigation.stats.totalDistanceKm.toLocaleString('fa-IR')} کیلومتر`}
                    />
                    <InfoRow
                      compact
                      icon={<Clock3 size={17} />}
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                <SmallStat
                  ...
                  label="سرعت لحظه‌ای"
                  value={`${navigation.stats.currentSpeed.toLocaleString('fa-IR')} km/h`}
                />
                <SmallStat
                  icon={<Fuel size={18} className="text-emerald-500" />}
                  label="مصرف سوخت"
                  value={`${navigation.stats.fuelConsumedLiters.toLocaleString('fa-IR')} لیتر`}
                />
                <SmallStat
                ...
                <SmallStat
                  ...
                  value={`${navigation.stats.efficiencyScore.toLocaleString('fa-IR')} از ۱۰۰`}
                />
              </div>
              ...
              {status === 'assigned' && (
                ...
                >
                  {isSubmitting ? (
                    ...
                    <>
                      <Play size={19} fill="currentColor" /> شروع مأموریت و
                      ردیابی
                    </>
                  )}
                </button>
              ...
              {status === 'in_progress' && (
                ...
                >
                  {isSubmitting ? (
                    ...
                    <>
                      <Square size={19} fill="currentColor" /> پایان مأموریت و
                      ثبت عملکرد
                    </>
                  )}
                </button>
              ...
              {status === 'completed' && (
                <div className="flex items-center justify-center gap-2 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm font-bold text-emerald-700 dark:border-emerald-900/50 dark:bg-emerald-950/30 dark:text-emerald-300">
                  <BadgeCheck size={19} /> این مأموریت با موفقیت تکمیل گردید.
                </div>
              )}
            </div>
          ) : activeTab === 'route' ? (
                  ...
                  <div className="mt-4">
                    {navigation.isDriving ? (
                      <button
                        type="button"
                        onClick={() => void handleComplete()}
                        className="flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-rose-600 font-bold text-white"
                      >
                        <Square size={18} fill="currentColor" /> توقف و ثبت
                        پایان مأموریت
                      </button>
                    ) : (
                      <button
                        type="button"
                        onClick={() => void handleStart()}
                        className="flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 font-bold text-white"
                      >
                        <Play size={18} fill="currentColor" /> شروع حرکت به سمت
                        مقصد
                      </button>
                    )}
                  </div>
            ...
            <div className="flex flex-col gap-5">
              <section className="rounded-3xl border border-neutral-200 bg-white p-5 dark:border-neutral-800 dark:bg-neutral-900">
                ...
                <div className="mt-5 grid grid-cols-2 gap-3 border-t border-neutral-100 pt-5 dark:border-neutral-800">
                  ...
                  />
                  <SmallStat
                    icon={<BadgeCheck size={17} className="text-emerald-500" />}
                    label="خودروی جاری"
                    value={vehicle?.plateNumber || 'تخصیص نشده'}
                  />
                </div>
              </section>
              <section className="rounded-3xl border border-neutral-200 bg-white p-5 dark:border-neutral-800 dark:bg-neutral-900">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs text-neutral-500 dark:text-neutral-400">
                      {navigation.isDriving
                        ...
                        : 'برای شروع ردیابی، مأموریت را آغاز کنید.'}
                    </p>
                  </div>
                </div>
              </section>
...
function TrackingCard({
  ...
  return (
    <section className="rounded-3xl border border-neutral-200 bg-white p-5 dark:border-neutral-800 dark:bg-neutral-900">
      ...
      <div
        className={`mt-4 rounded-2xl p-3 text-xs font-semibold ${error ? 'bg-rose-50 text-rose-700 dark:bg-rose-950/30 dark:text-rose-300' : active ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-300' : 'bg-neutral-50 text-neutral-600 dark:bg-neutral-950 dark:text-neutral-400'}`}
      >
        <span
          className={`mr-2 inline-block h-2.5 w-2.5 rounded-full ${error ? 'bg-rose-500' : active ? 'animate-pulse bg-emerald-500' : 'bg-neutral-400'}`}
        />
        {error ||
          (active
...
function BottomNavigation({
  ...
  const items = [
    ...
    { id: 'route' as const, label: 'نقشه و مسیر', icon: <Map size={20} /> },
    { id: 'profile' as const, label: 'پروفایل', icon: <UserRound size={20} /> },
  ];
  return (
      ...
      <div className="mx-auto grid w-full max-w-md grid-cols-3 gap-2">
        {items.map((item) => (
          <button
            ...
            className={`flex min-h-14 flex-col items-center justify-center gap-1 rounded-2xl text-xs font-bold transition ${activeTab === item.id ? 'bg-orange-50 text-orange-600 dark:bg-orange-950/40 dark:text-orange-400' : 'text-neutral-500 hover:bg-neutral-50 dark:text-neutral-400 dark:hover:bg-neutral-800'}`}
          >
            {item.icon}
            {item.label}
          </button>
...
function InfoRow({
  ...
  return (
    <div className={`flex gap-3 ${compact ? 'items-start' : 'items-center'}`}>
      <div
        className={`flex shrink-0 items-center justify-center rounded-xl ${compact ? 'h-9 w-9' : 'h-10 w-10'} ${iconClassName || 'bg-orange-50 text-orange-600 dark:bg-orange-950/30 dark:text-orange-400'}`}
      >
        {icon}
      </div>
      <div className="min-w-0">
        ...
        <p
          className={`mt-0.5 break-words font-bold text-neutral-800 dark:text-neutral-100 ${compact ? 'text-xs' : 'text-sm'}`}
        >
          {value}
        </p>
      </div>
